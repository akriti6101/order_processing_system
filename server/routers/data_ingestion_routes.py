...
async def add_customers(customer:Customer=Body(...),db_con=Depends(get_db_connection)):
    try:
        ...
        raise HTTPException(status_code=400,details="Could not save customer details")
...
async def get_customers(db_con=Depends(get_db_connection)):
    try:
        ...
        raise HTTPException(status_code=400,details="Could not fetch customers")
...
async def add_item(item:Item=Body(...),db_con=Depends(get_db_connection)):
    try:
        ...
        raise HTTPException(status_code=400,details="Could not save item details")
...
async def get_items(db_con=Depends(get_db_connection)):
    try:
        ...
        raise HTTPException(status_code=400,details="Could not fetch items")
